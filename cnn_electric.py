...
# don't use resample fn
max_idx = len(global_active_power) // (24*60)
global_active_power_daily = pd.DataFrame(global_active_power[:max_idx*24*60].values.reshape(-1, 24*60).mean(axis=1))
...
filter_sizes = [2, 4, 8, 16, 32, 64, 128]
kernel_sizes = [1, 2, 4, 8]

validation_mses = [[] for _ in range(len(filter_sizes))]
validation_maes = [[] for _ in range(len(filter_sizes))]
validation_rmses = [[] for _ in range(len(filter_sizes))]

models = [[] for _ in range(len(filter_sizes))]
import time 
...
for i, filter_size in enumerate(filter_sizes):
    for kernel_size in kernel_sizes:
        ...
        model = tf.keras.Sequential([
            # mlp
            # input is (batch, 21, 1)
            # reshape to (batch, 21-7, 1)
            tf.keras.layers.Conv1D(filter_size, kernel_size, activation="relu"),
        ...
        history = model.fit(input_datas, output_datas, epochs=300, batch_size=32, validation_data=(val_input_datas, val_output_datas), callbacks=[tf.keras.callbacks.EarlyStopping(patience=10)])

        mse = history.history["val_loss"][-1]
        mae = history.history["val_mae"][-1]
        rmse = history.history["val_root_mean_squared_error"][-1]

        validation_mses[i].append(mse)
        validation_maes[i].append(mae)
        validation_rmses[i].append(rmse)

        models[i].append(model)
after_cv = time.time()

print(f"CV Time: {(after_cv-before_cv)/(len(filter_sizes)*len(kernel_sizes))} sec/model")

# print min over filter sizes
min_mses = [min(validation_mses[i]) for i in range(len(filter_sizes))]
min_maes = [min(validation_maes[i]) for i in range(len(filter_sizes))]
min_rmses = [min(validation_rmses[i]) for i in range(len(filter_sizes))]

print(min_mses)
print(min_maes)
print(min_rmses)

idx1 = input("Which filter size to use? ")

filt_mse = validation_mses[int(idx1)]
filt_mae = validation_maes[int(idx1)]
filt_rmse = validation_rmses[int(idx1)]

print(filt_mse)
print(filt_mae)
print(filt_rmse)

idx2 = input("Which kernel size to use? ")

plt.plot(history.history['loss'])
plt.show()

model = models[int(idx1)][int(idx2)]
...
plt.title("Seven-Day Forecasts on Holdout Data (Conv)")
plt.show()
...
plt.title("Weighted-Average Forecasts on Holdout Data (Conv)")
plt.show()
