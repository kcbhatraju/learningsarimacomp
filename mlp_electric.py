...
for hidden_layer_size in hidden_layer_sizes:
    ...
    model = tf.keras.Sequential([
        # mlp
        # input is (batch, time, 1)
        # reshape to (batch, time)
        tf.keras.layers.Flatten(input_shape=(15, 1)),
        tf.keras.layers.Dense(hidden_layer_size, activation="relu"),
        # output is (batch, 7)
        tf.keras.layers.Dense(out_steps),
        # reshape to (batch, 7, 1)
        tf.keras.layers.Reshape((out_steps, 1))
    ])

    """model.compile(optimizer="adam", loss="mse")
    history = model.fit(input_datas, output_datas, epochs=300, batch_size=32)"""

    # use early stopping (patience=10)
    # track mse, mae, and rmse
...
after_cv = time.time()
print(f"CV time: {(after_cv-before_cv)/len(hidden_layer_sizes)} sec/model")
...
plt.title("Seven-Day Forecasts on Holdout Data (MLP)")
plt.show()
...
plt.title("Weighted-Average Forecasts on Holdout Data (MLP)")
plt.show()
